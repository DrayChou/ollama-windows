...
class OllamaManager:
    ...
    def create_status_tab(self):
        ...
        ttk.Label(left_frame, text="运行状态:").grid(row=0, column=0, sticky='w', padx=(0,10))
        self.status_label = ttk.Label(left_frame, text="检查中...", foreground='orange')
        self.status_label.grid(row=0, column=1, sticky='w')
        
        ttk.Label(left_frame, text="安装路径:").grid(row=1, column=0, sticky='w', padx=(0,10))
        self.path_label = ttk.Label(left_frame, text="未检测到")
        self.path_label.grid(row=1, column=1, sticky='w')
        
        ttk.Label(left_frame, text="当前版本:").grid(row=2, column=0, sticky='w', padx=(0,10))
        self.version_label = ttk.Label(left_frame, text="未知")
        self.version_label.grid(row=2, column=1, sticky='w')
        
        ttk.Label(left_frame, text="最新版本:").grid(row=3, column=0, sticky='w', padx=(0,10))
        ...
        self.start_btn = ttk.Button(right_frame, text="启动", command=self.start_ollama)
        self.start_btn.pack(pady=2, fill='x')
        
        self.stop_btn = ttk.Button(right_frame, text="停止", command=self.stop_ollama)
        self.stop_btn.pack(pady=2, fill='x')
        
        self.restart_btn = ttk.Button(right_frame, text="重启", command=self.restart_ollama)
        self.restart_btn.pack(pady=2, fill='x')
        
        ttk.Separator(right_frame, orient='horizontal').pack(fill='x', pady=5)
        
        self.refresh_btn = ttk.Button(right_frame, text="刷新状态", command=self.refresh_status)
        ...
        self.update_status_label = ttk.Label(update_info_frame, text="点击检查更新")
        self.update_status_label.pack(side='left')
        
        update_btn_frame = ttk.Frame(update_info_frame)
        update_btn_frame.pack(side='right')
        
        self.check_update_btn = ttk.Button(update_btn_frame, text="检查更新", command=self.check_updates)
        self.check_update_btn.pack(side='left', padx=2)
        
        self.install_btn = ttk.Button(update_btn_frame, text="安装/更新", command=self.install_ollama, state='disabled')
        ...
        # GPU 信息树形视图
        self.gpu_tree = ttk.Treeview(hw_frame, columns=('name', 'type', 'memory', 'status'), show='headings', height=6)
    ...
    def create_config_tab(self):
        ...
        # Ollama 路径
        ttk.Label(path_frame, text="Ollama 程序路径:").grid(row=0, column=0, sticky='w', padx=(0,10))
        self.ollama_path_var = tk.StringVar()
        ttk.Entry(path_frame, textvariable=self.ollama_path_var, width=50).grid(row=0, column=1, sticky='ew', padx=(0,5))
        ttk.Button(path_frame, text="浏览", command=self.browse_ollama_path).grid(row=0, column=2)
        
        # Models 路径
        ttk.Label(path_frame, text="模型存储路径:").grid(row=1, column=0, sticky='w', padx=(0,10), pady=(5,0))
        self.models_path_var = tk.StringVar()
        ttk.Entry(path_frame, textvariable=self.models_path_var, width=50).grid(row=1, column=1, sticky='ew', padx=(0,5), pady=(5,0))
        ttk.Button(path_frame, text="浏览", command=self.browse_models_path).grid(row=1, column=2, pady=(5,0))
        ...
        # OLLAMA_HOST
        ttk.Label(env_frame, text="OLLAMA_HOST:").grid(row=0, column=0, sticky='w', padx=(0,10))
        self.host_var = tk.StringVar(value="0.0.0.0")
        ttk.Entry(env_frame, textvariable=self.host_var, width=20).grid(row=0, column=1, sticky='w')
        ttk.Label(env_frame, text="(设置为 0.0.0.0 允许外部访问)").grid(row=0, column=2, sticky='w', padx=(10,0))
        
        # OLLAMA_PORT
        ttk.Label(env_frame, text="OLLAMA_PORT:").grid(row=1, column=0, sticky='w', padx=(0,10), pady=(5,0))
        self.port_var = tk.StringVar(value="11434")
        ttk.Entry(env_frame, textvariable=self.port_var, width=20).grid(row=1, column=1, sticky='w', pady=(5,0))
        ttk.Label(env_frame, text="(默认端口 11434)").grid(row=1, column=2, sticky='w', padx=(10,0), pady=(5,0))
        
        # HSA_OVERRIDE_GFX_VERSION (AMD)
        ttk.Label(env_frame, text="HSA_OVERRIDE_GFX_VERSION:").grid(row=2, column=0, sticky='w', padx=(0,10), pady=(5,0))
        self.hsa_var = tk.StringVar()
        ttk.Entry(env_frame, textvariable=self.hsa_var, width=20).grid(row=2, column=1, sticky='w', pady=(5,0))
        ttk.Label(env_frame, text="(AMD 集显支持，通常设置为 11.0.0)").grid(row=2, column=2, sticky='w', padx=(10,0), pady=(5,0))
        ...
        self.args_text = scrolledtext.ScrolledText(advanced_frame, height=4, width=50)
        ...
        ttk.Button(btn_frame, text="保存配置", command=self.save_config).pack(side='left', padx=(0,5))
        ttk.Button(btn_frame, text="重置配置", command=self.reset_config).pack(side='left', padx=5)
        ttk.Button(btn_frame, text="应用并重启", command=self.apply_and_restart).pack(side='left', padx=5)
    ...
    def create_models_tab(self):
        ...
        # 模型列表
        self.models_tree = ttk.Treeview(left_frame, columns=('size', 'modified'), show='tree headings', height=15)
        ...
        # 添加滚动条
        models_scroll = ttk.Scrollbar(left_frame, orient='vertical', command=self.models_tree.yview)
        ...
        ttk.Button(models_btn_frame, text="刷新列表", command=self.refresh_models).pack(side='left', padx=(0,5))
        ttk.Button(models_btn_frame, text="删除模型", command=self.delete_model).pack(side='left', padx=5)
        ttk.Button(models_btn_frame, text="模型详情", command=self.show_model_info).pack(side='left', padx=5)
        ...
        ttk.Button(search_frame, text="搜索", command=self.search_models).pack(side='left')
        ...
        model_entry = ttk.Entry(download_frame, textvariable=self.model_name_var)
        model_entry.pack(side='left', fill='x', expand=True, padx=(5,5))
        ttk.Button(download_frame, text="下载", command=self.download_model).pack(side='left')
        ...
        for i, (model, size) in enumerate(popular_models):
            ...
            col = i % 3
            btn_text = f"{model}\n({size})"
            btn = ttk.Button(quick_frame, text=btn_text, 
    ...
    def create_chat_tab(self):
        ...
        self.chat_model_combo = ttk.Combobox(model_frame, textvariable=self.chat_model_var, state='readonly')
        self.chat_model_combo.pack(side='left', padx=(5,10), fill='x', expand=True)
        
        ttk.Button(model_frame, text="刷新", command=self.refresh_chat_models).pack(side='left')
        ...
        # 对话历史
        self.chat_history = scrolledtext.ScrolledText(chat_frame, height=20, state='disabled')
        ...
        self.chat_input.pack(side='left', fill='x', expand=True, padx=(5,5))
        
        # 按钮
        btn_frame = ttk.Frame(input_frame)
        btn_frame.pack(side='right', fill='y')
        
        ttk.Button(btn_frame, text="发送", command=self.send_message).pack(fill='x', pady=(0,2))
        ttk.Button(btn_frame, text="清空", command=self.clear_chat).pack(fill='x')
    ...
    def create_logs_tab(self):
        ...
        ttk.Button(log_type_frame, text="刷新", command=self.refresh_logs).pack(side='left')
        ttk.Button(log_type_frame, text="清空", command=self.clear_logs).pack(side='left', padx=(5,0))
        ttk.Button(log_type_frame, text="导出", command=self.export_logs).pack(side='left', padx=(5,0))
        
        # 日志显示区域
        self.log_text = scrolledtext.ScrolledText(tab, height=25, font=('Consolas', 9))
        ...
        def check_status():
            try:
                ...
                if self.ollama_path and not self.models_path:
                    self.models_path = os.path.join(os.path.dirname(self.ollama_path), "models")
        ...
        def get_gpu_info():
            try:
                ...
                try:
                    ...
                    for gpu in gpus:
                        if gpu.Name and not any(x in gpu.Name.lower() for x in ['basic', 'generic']):
                            # 显存大小
                            memory = "未知"
                            if gpu.AdapterRAM:
                            ...
                            if any(x in gpu.Name.lower() for x in ['nvidia', 'geforce', 'rtx', 'gtx']):
                                gpu_type = "NVIDIA"
                                support_status = "支持 (CUDA)"
                            elif any(x in gpu.Name.lower() for x in ['amd', 'radeon']):
                                gpu_type = "AMD"
                                if any(x in gpu.Name.lower() for x in ['integrated', 'graphics', 'vega']):
                                    support_status = "集显 (需配置)"
                                else:
                                    support_status = "支持 (ROCm)"
                            elif any(x in gpu.Name.lower() for x in ['intel', 'uhd', 'iris']):
                                gpu_type = "Intel"
                                support_status = "不支持"
                            
                            # 添加到树形视图
                            self.gpu_tree.insert('', 'end', values=(gpu.Name, gpu_type, memory, support_status))
                            gpu_found = True
                        ...
                        if result.stdout:
                            ...
                            for line in lines:
                                if '|' in line:
                                    ...
                                    if any(x in name.lower() for x in ['nvidia', 'geforce', 'rtx', 'gtx']):
                                        gpu_type = "NVIDIA"
                                        support_status = "支持 (CUDA)"
                                    elif any(x in name.lower() for x in ['amd', 'radeon']):
                                        gpu_type = "AMD"
                                        if any(x in name.lower() for x in ['integrated', 'graphics', 'vega']):
                                            support_status = "集显 (需配置)"
                                        else:
                                            support_status = "支持 (ROCm)"
                                    elif any(x in name.lower() for x in ['intel', 'uhd', 'iris']):
                                        gpu_type = "Intel"
                                        support_status = "不支持"
                                    
                                    self.gpu_tree.insert('', 'end', values=(name.strip(), gpu_type, memory.strip(), support_status))
                                    gpu_found = True
                ...
                # 如果都失败了，显示提示信息
                if not gpu_found:
                    self.gpu_tree.insert('', 'end', values=("无法检测显卡信息", "请检查系统", "", "可能需要管理员权限"))
            ...
            try:
                ...
                if self.current_version:
                    if self.current_version == self.latest_version:
                        ...
                        self.update_status_label.config(text=f"发现新版本: {self.latest_version}")
                        self.install_btn.config(state='normal')
                else:
                    self.update_status_label.config(text="未检测到 Ollama，可以安装")
                    self.install_btn.config(state='normal')
    ...
    def install_ollama(self):
        ...
        if messagebox.askyestion("确认", f"确定要安装/更新到版本 {self.latest_version} 吗？"):
            def install():
                try:
                    self.install_btn.config(state='disabled')
                    ...
                    # 下载最新版本
                    download_url = f"https://github.com/ollama/ollama/releases/download/v{self.latest_version}/ollama-windows-amd64.zip"
                    ...
                    with tempfile.TemporaryDirectory() as temp_dir:
                        ...
                        if not target_path:
                            target_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ollama.exe")
    ...
    def refresh_models(self):
        ...
        def get_models():
            try:
                ...
                # 检查 Ollama 是否运行
                ollama_running = any('ollama' in proc.info['name'].lower() 
                        ...
                        if result.stdout:
                            ...
                            for line in lines:
                                if line.strip():
                                    ...
                                    if len(parts) >= 3:
                                        name = parts[0]
                                        size = parts[2] if len(parts) > 2 else "未知"
                                        modified = " ".join(parts[3:]) if len(parts) > 3 else "未知"
                                        
                                        self.models_tree.insert('', 'end', text=name, values=(size, modified))
                    
                    except subprocess.TimeoutExpired:
                        self.models_tree.insert('', 'end', text="获取超时", values=("", ""))
                    except Exception as e:
                        self.models_tree.insert('', 'end', text=f"错误: {str(e)}", values=("", ""))
                else:
                    self.models_tree.insert('', 'end', text="Ollama 未运行", values=("", ""))
                ...
                try:
                    if self.ollama_path:
                        ...
                        if result.returncode == 0:
                            ...
                            error_msg = result.stderr or result.stdout or "删除失败"
            ...
            try:
                if self.ollama_path:
                    ...
                    if result.stdout:
                        ...
                        text_widget = scrolledtext.ScrolledText(info_window, font=('Consolas', 9))
    ...
    def search_models(self):
        ...
        if search_term:
            ...
            result = "\n".join(suggestions)
            messagebox.showinfo("搜索建议", f"基于 '{search_term}' 的建议:\n\n{result}")
            ...
            try:
                if self.ollama_path:
                    ...
                    response = requests.post(
                        f"http://{self.host_var.get() or 'localhost'}:{self.port_var.get() or '11434'}/api/generate",
                        json=data,
                        timeout=60
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        self.add_chat_message("系统", f"请求失败: {response.status_code}")
